...
export function takeFeedback(db: AppDatabase, takeId: string, kind: 'coach' | 'delta') {
  return db
    .select()
    .from(feedback)
    .where(and(eq(feedback.takeId, takeId), eq(feedback.kind, kind)))
    .get();
}
...
export function saveCoach(
  ...
  return db.transaction((tx) => {
    ...
    if (dayTakes(tx, dayId).find((take) => take.state === 'saved')?.id !== takeId)
      throw new Error('Coach must analyse the first saved take.');
    ...
    tx.insert(feedback)
      .values({
        ...
        wordChoice: result.scores.word_choice,
        pacePausing: result.scores.pace_pausing,
        flow: result.scores.flow,
        presence: result.scores.presence,
        paceWpm: result.transcript.trim().split(/\s+/).length / (durationS / 60),
      })
      .run();
    result.corrections.forEach((fix, i) =>
      tx
        .insert(corrections)
        .values({
          id: id + '-' + fix.id,
    ...
    tx.insert(followups)
      .values({
        ...
        dayId,
        sourceTakeId: takeId,
        mode: day.policy.interaction,
        question: result.hidden_follow_up,
        promptVersion: COACH_PROMPT_VERSION,
      })
      .run();
    if (result.word_for_today)
      tx.insert(vocab)
        .values({
          id: dayId + '-word',
          dayId,
          word: result.word_for_today.word,
          meaning: result.word_for_today.meaning,
          exampleSentence: result.word_for_today.use_it_here,
        })
        .onConflictDoNothing()
        .run();
  });
...
export function saveDelta(
  ...
  if (!saved.slice(1).some((take) => take.id === takeId))
    throw new Error('A saved reshoot is required.');
  const result = validateDelta(raw, original.result);
  if (!takeFeedback(db, takeId, 'delta'))
    db.insert(feedback)
      .values({
        id: newId('delta'),
        dayId,
        ...
        kind: 'delta',
        model,
        promptVersion: 'delta-1',
        schemaVersion: 'delta-1',
        rubricVersion: COACH_RUBRIC_VERSION,
        transcript: result.transcript,
        resultJson: result,
      })
...
export function comparison(db: AppDatabase, dayId: string) {
  const saved = dayTakes(db, dayId).filter((take) => take.state === 'saved');
  return saved.slice(1).map((take) => {
    const row = takeFeedback(db, take.id, 'delta');
    return { take, result: row ? deltaSchema.parse(row.resultJson) : null };
  });
}
export function revealedFollowup(db: AppDatabase, dayId: string) {
  return db
    .select()
    .from(followups)
    .where(eq(followups.dayId, dayId))
    .orderBy(asc(followups.createdAt))
    .get();
}
export function revealFollowup(db: AppDatabase, dayId: string) {
  assertDayUnlocked(db, dayId);
  if (dayTakes(db, dayId).filter((take) => take.state === 'saved').length < 2)
    throw new Error('Save Take 2 before revealing the question.');
  const item = revealedFollowup(db, dayId);
  if (!item) throw new Error('Analyse Take 1 first.');
  if (!item.revealedAt)
    db.update(followups).set({ revealedAt: new Date() }).where(eq(followups.id, item.id)).run();
