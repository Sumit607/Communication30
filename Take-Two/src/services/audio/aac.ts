...
export function inspectAac(base64: string): VerifiedAudio {
  if (!base64 || base64.length > 14 * 1024 * 1024)
    throw new Error('Audio exceeds the upload limit.');
  ...
  while (offset < raw.length) {
    ...
    if (
      ...
      (b(6) & 3) !== 0 ||
      size <= header ||
      offset + size > raw.length
    )
      throw new Error('Audio frame validation failed.');
    if (frames && (rate !== sampleRate || channels !== channelCount))
      throw new Error('Audio format changed during capture.');
    sampleRate = rate;
  ...
  if (!frames || durationS > 605 || raw.length > 10 * 1024 * 1024)
    throw new Error('Audio is empty or too long.');
