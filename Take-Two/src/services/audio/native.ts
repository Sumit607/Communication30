...
type MediaModule = {
  extractAudio(uri: string): Promise<{ base64: string }>;
  duration(uri: string): Promise<number>;
};
function native() {
...
export async function extractAudio(uri: string) {
  return inspectAac((await native().extractAudio(uri)).base64);
}
export async function recordingDuration(uri: string) {
  return native().duration(uri);
}
