...
export function publicAiError(error: unknown) {
  ...
  if (status === 429) return 'Gemini quota reached. Your work is saved. Retry later; no paid fallback was used.';
  if (status === 401 || status === 403) return 'Gemini access was denied. Check your API key and project in Settings.';
  if (status === 404) return 'This model is unavailable. Choose an accessible free-tier model in Settings.';
...
export async function requestJson<T>({ apiKey, model, schema, system, data, audio, signal }: {
  apiKey: string; model: string; schema: z.ZodType<T>; system: string; data: unknown; audio?: VerifiedAudio; signal?: AbortSignal;
}): Promise<T> {
  if (!/^gemini-[a-zA-Z0-9._-]+$/.test(model)) throw new Error('Configure a Gemini model in Settings.');
  const verified = audio ? inspectAac(audio.base64) : undefined;
  const ai = new GoogleGenAI({ apiKey, httpOptions: { timeout: 90000, retryOptions: { attempts: 1 } } });
  const response = await ai.models.generateContent({ model, contents: [{ role: 'user', parts: [
    { text: JSON.stringify(data) },
    ...(verified ? [{ inlineData: { mimeType: verified.mimeType, data: verified.base64 } }] : []),
  ] }], config: { systemInstruction: system, responseMimeType: 'application/json', responseJsonSchema: z.toJSONSchema(schema), maxOutputTokens: 8192, abortSignal: signal } });
